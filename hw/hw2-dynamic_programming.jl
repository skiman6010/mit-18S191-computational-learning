...
Removing 2 copies of slices per loop run (342 * 2) should remove 684 allocations. 
"""
...
least_energy is called at every pixel in the path for each loop, having already calculated energy at the same pixels in the path.
...
# ╔═╡ 5edea886-fb7e-11ea-1b96-c3086caf1208
"Modified dict from Discord to perform better."
if shrink_dict
    img2 = decimate(img, 4)
    dict_carved = shrink_n(img2, 20, recursive_memoized_seam)
    md"Shrink by: $(@bind dict_n Slider(1:20, show_value=true))"
end
